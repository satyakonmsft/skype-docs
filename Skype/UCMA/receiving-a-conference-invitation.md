...
description: An overview of receiving a conference invitation.
...
At the SIP level, a [Call](/dotnet/api/microsoft.rtc.collaboration.call?view=ucma-api&preserve-view=true) is represented as a SIP INVITE with Session Description Protocol (SDP) that contains information about the media types that are included in the call, together with other information that helps negotiate a connection between the sender and recipient of the INVITE.
...
1.  The application registers for the [ConferenceInvitationReceived](https://msdn.microsoft.com/library/hh366294\(v=office.16\)) event (an event on the [LocalEndpoint](/dotnet/api/microsoft.rtc.collaboration.localendpoint?view=ucma-api&preserve-view=true) class).
